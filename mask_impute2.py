...
Masked AE imputation using SPLAT cell-aware bio-zero predictions with the
same logic as splat_predict2.py, but fixed to mode "none" and thr_drop=0.8200.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Masked AE imputation using SPLAT cell-aware (mode none).")
    parser.add_argument("input_path", help="Path to .rds file or directory")
    parser.add_argument("output_dir", help="Output directory for mask_impute2_mse_table.tsv")
    ...
    _write_table(output_dir / "mask_impute2_raw_mse_table.tsv", results_raw)
    _write_table(output_dir / "mask_impute2_no_biozero_mse_table.tsv", results_no_bio)
    _write_table(output_dir / "mask_impute2_mse_table.tsv", results_final)

    print(f"Wrote {output_dir / 'mask_impute2_raw_mse_table.tsv'}")
    print(f"Wrote {output_dir / 'mask_impute2_no_biozero_mse_table.tsv'}")
    print(f"Wrote {output_dir / 'mask_impute2_mse_table.tsv'}")
