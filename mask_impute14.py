...
without blend, plus exhaustive scoring over zero_iso/dropout_iso/constrained_zero
while keep_positive remains enabled.
"""
...
COMBO_COMPONENTS = ("zero_iso", "dropout_iso", "constrained_zero")
...
def run_combo(
    ...
    p_bio: np.ndarray,
    use_zero_iso: bool,
    use_dropout_iso: bool,
    use_constrained_zero: bool,
) -> Dict[str, float]:
    ...
    if use_zero_iso and float(CONFIG["zero_iso_weight"]) > 0.0:
    ...
    if use_dropout_iso and float(CONFIG["dropout_iso_weight"]) > 0.0:
    ...
    if use_constrained_zero and bool(CONFIG["constrained_zero_scale"]):
    ...
    return compute_mse_metrics(log_imputed_final, ds["log_true"], ds["logcounts"])
...
def evaluate_combo(
    ...
    for ds in datasets:
        metrics = run_combo(
            ...
            p_bio=ds["p_bio"],
            use_zero_iso=use_zero_iso,
            use_dropout_iso=use_dropout_iso,
            use_constrained_zero=use_constrained_zero,
        )
...
def write_combo_table(path: Path, rows: List[Dict[str, object]]) -> None:
    header = [
        "combo_id",
        "components",
        "zero_iso",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Score all combinations of zero_iso/dropout_iso/constrained_zero "
            "with keep_positive always enabled (blend removed)."
        )
    )
    ...
    for idx, combo in enumerate(itertools.product([False, True], repeat=3), start=1):
        use_zero_iso, use_dropout_iso, use_constrained_zero = combo
        avg_mse, avg_bz, score = evaluate_combo(
            datasets,
            use_zero_iso=use_zero_iso,
            use_dropout_iso=use_dropout_iso,
            use_constrained_zero=use_constrained_zero,
        ...
        enabled = [
            name
            for name, flag in zip(COMBO_COMPONENTS, combo)
            if flag
        ]
        rows.append(
            {
                ...
                "zero_iso": bool(use_zero_iso),
                "dropout_iso": bool(use_dropout_iso),
                "constrained_zero": bool(use_constrained_zero),
                "avg_mse": avg_mse,
                "avg_bz": avg_bz,
                "score": score,
            }
    ...
    write_combo_table(output_dir / "mask_impute14_combo_scores.tsv", rows)

    best = rows[0] if rows else None
    print("\n=== mask_impute14 ===")
    print("Fixed components: keep_positive=True, hard_zero_bio=True (blend removed)")
    if best is not None:
        print(
            "Best combo: "
            f"{best['components']} | avg_mse={best['avg_mse']:.6f} "
            f"avg_bz={best['avg_bz']:.6f} score={best['score']:.6f}"
        )
