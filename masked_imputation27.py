...
Adds train-time refinement of biozero/dropout probabilities.
"""
...
REFINE_PARAMS = {
    ...
    "dropout_scale": 0.30,
    "update_min_dropout_prob": 0.98,
    "force_dropout_prob": 0.9995,
    "prior_floor_ratio": 0.20,
    "p_bio_min": 1e-4,
    "p_bio_max": 0.999,
}
...
def _estimate_dropout_prob_from_recon(
...
def _refine_bio_probabilities(
    ...
    p_dropout = _estimate_dropout_prob_from_recon(
        recon_log,
        zeros_obs,
...
def train_autoencoder_reconstruct(
    ...
    *,
    ...
    refine_start = max(1, int(REFINE_PARAMS["start_epoch"]))
    refine_every = max(1, int(REFINE_PARAMS["every_n_epochs"]))
    ...
    if use_full_gpu:
        ...
        loader_workers = 0 if refine_active else (int(num_workers) if fast_mode else 0)
        loader = DataLoader(
            ...
            persistent_workers=bool(fast_mode) and loader_workers > 0,
        )
    ...
    def _sync_bio_mask() -> None:
        bio_np = np.clip(bio_prob, 0.0, 1.0).astype(np.float32, copy=False)
        bio_t = torch.as_tensor(bio_np, dtype=torch.float32, device=bio_mask.device)
        bio_mask.copy_(bio_t)

    def _predict_recon_log_all() -> np.ndarray:
        preds = []
        with torch.no_grad():
            if use_full_gpu:
                for i in range(0, Xtr.size(0), batch_size):
                    ...
                    preds.append(recon_orig.cpu().numpy())
            else:
                for i in range(0, Xtr.size(0), batch_size):
                    xb = Xtr[i : i + batch_size].to(device)
                    recon = model(xb)
                    recon_np = recon.cpu().numpy()
                    preds.append(scaler.inverse_transform(recon_np))
        if not preds:
            return np.zeros_like(logcounts, dtype=np.float32)
        return np.vstack(preds).astype(np.float32, copy=False)
    ...
    for epoch_idx in range(1, epochs + 1):
        ...
        if refine_active and epoch_idx >= refine_start:
            if ((epoch_idx - refine_start) % refine_every == 0) or (epoch_idx == epochs):
                model.eval()
                recon_probe = _predict_recon_log_all()
                bio_prob = _refine_bio_probabilities(
                    p_bio_curr=bio_prob,
                    p_bio_prior=bio_prob_prior,
                    recon_log=recon_probe,
                    zeros_obs=zeros_obs,
                )
                _sync_bio_mask()
                model.train()
...
def main() -> None:
    ...
    _write_table(
        ...
        ["avg_mse", "avg_bz_mse", "score", "runtime_sec", "fast_mode", "refine_enabled"],
        [
            {
                "avg_mse": avg_mse,
                "avg_bz_mse": avg_bz,
