...
to drive down biozero MSE while keeping overall MSE low. Adds unsupervised
expression-based and kNN-latent biozero heuristics alongside SPLAT posteriors,
with optional semi-supervised calibration of p_bio thresholds (global, gene-quantile,
or bin-shrink).
"""
...
def main() -> None:
    ...
    parser.add_argument("output_dir", help="Output directory for mask_impute6_*_mse_table.tsv")
    ...
    if calibrate_p_bio:
        print(
            ...
            f"frac_scales={calib_frac_scale_list} bins={calibrate_bin_count} lambda={calibrate_lambda_mse}"
        )
    ...
    _write_table(output_dir / "mask_impute6_raw_mse_table.tsv", best_raw)
    _write_table(output_dir / "mask_impute6_no_biozero_mse_table.tsv", best_no_bio)
    _write_table(output_dir / "mask_impute6_mse_table.tsv", best_final)

    tuning_path = output_dir / "mask_impute6_tuning.tsv"
    tuning_cols = [
        "config_id",
        "objective",
        "avg_mse_biozero",
    ...
    print(
        ...
        f"calib_p_bio={calibrate_p_bio} calib_mode={calibrate_p_bio_mode} "
        f"calib_bins={calibrate_bin_count} calib_lambda={calibrate_lambda_mse} | obj={best_obj:.6f} "
        f"avg_bz={best_avg_bz:.6f} avg_mse={best_avg_mse:.6f}"
    )
    print(f"Wrote {output_dir / 'mask_impute6_raw_mse_table.tsv'}")
    print(f"Wrote {output_dir / 'mask_impute6_no_biozero_mse_table.tsv'}")
    print(f"Wrote {output_dir / 'mask_impute6_mse_table.tsv'}")
