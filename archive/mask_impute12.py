...
def _apply_zero_iso_scale(
    ...
    mode_norm = str(mode).lower()
    if mode_norm not in ("global", "gene"):
        raise ValueError(f"Unsupported zero-iso mode: {mode}")
    p_use = np.clip(p_bio, 0.0, 1.0) ** float(gamma)
    ...
    def _fit_bins(
        z_all: np.ndarray, x_all: np.ndarray, y_all: np.ndarray
    ) -> Tuple[np.ndarray, np.ndarray, float]:
        if z_all.size == 0:
            return np.array([], dtype=np.float32), np.array([], dtype=np.int32), float("nan")
        bin_idx, edges = _assign_quantile_bins(z_all, bins)
        if max_samples > 0 and z_all.size > max_samples:
            idx = rng.choice(z_all.size, size=max_samples, replace=False)
            x_fit = x_all[idx]
            y_fit = y_all[idx]
            bin_idx_fit = bin_idx[idx]
        else:
            x_fit = x_all
            y_fit = y_all
            bin_idx_fit = bin_idx
    ...
    if mode_norm == "global":
        ...
        if not np.any(mask):
            return log_imputed_final, float("nan")
        z = p_use[mask].ravel()
        x = log_imputed_final[mask].ravel()
        y = log_true[mask].ravel()
        scales, bin_idx, mean_scale = _fit_bins(z, x, y)
        if scales.size == 0:
            return log_imputed_final, float("nan")
        scale = scales[bin_idx]
        scale_blend = (1.0 - weight) + weight * scale
        log_adj[mask] = log_adj[mask] * scale_blend
        return log_adj, mean_scale
...
def _apply_dropout_iso_scale(
    ...
    mode_norm = str(mode).lower()
    if mode_norm not in ("global", "gene"):
        raise ValueError(f"Unsupported dropout-iso mode: {mode}")
    p_max = float(p_max)
    if p_max <= 0.0:
        return log_imputed_final, float("nan")
    p_use = np.clip(p_bio, 0.0, 1.0) ** float(gamma)
    ...
    def _fit_bins(
        ...
        if max_samples > 0 and int(drop_mask.sum()) > max_samples:
            drop_idx = np.flatnonzero(drop_mask)
            chosen = rng.choice(drop_idx.size, size=max_samples, replace=False)
            sel = drop_idx[chosen]
            x_fit = x_all[sel]
            y_fit = y_all[sel]
            bin_idx_fit = bin_idx[sel]
        else:
            x_fit = x_all[drop_mask]
            y_fit = y_all[drop_mask]
            bin_idx_fit = bin_idx[drop_mask]
        sum_x2 = np.bincount(
            bin_idx_fit, weights=x_fit * x_fit, minlength=int(edges.size - 1)
        )
        sum_xy = np.bincount(
            bin_idx_fit, weights=x_fit * y_fit, minlength=int(edges.size - 1)
        )
        scales = np.ones((int(edges.size - 1),), dtype=np.float32)
        for i in range(scales.size):
            if sum_x2[i] > EPSILON:
    ...
    if mode_norm == "global":
        ...
        if not np.any(apply_mask):
            return log_imputed_final, mean_scale
...
def _apply_dual_scale(
    ...
    if weight <= 0.0:
        return log_imputed_final, float("nan")
    p_use = np.clip(p_bio, 0.0, 1.0) ** float(gamma)
    scale = float(s_drop) * (1.0 - p_use) + float(s_bio) * p_use
    scale_blend = (1.0 - weight) + weight * scale
    log_adj = log_imputed_final.copy()
    log_adj[zeros_obs] = log_adj[zeros_obs] * scale_blend[zeros_obs]
    mean_scale = float(np.nanmean(scale[zeros_obs])) if np.any(zeros_obs) else float("nan")
    return log_adj, mean_scale
...
def _is_valid_config(config: Dict[str, object]) -> bool:
    ...
    if float(config.get("recover_gamma", 1.0)) <= 0.0:
        return False
    zero_iso_weight = float(config.get("zero_iso_weight", 0.0))
    if zero_iso_weight < 0.0 or zero_iso_weight > 1.0:
        return False
    zero_iso_mode = str(config.get("zero_iso_mode", "global")).lower()
    if zero_iso_mode not in ("global", "gene"):
        return False
    if int(config.get("zero_iso_bins", 1)) < 1:
        return False
    if float(config.get("zero_iso_gamma", 1.0)) <= 0.0:
        return False
    if float(config.get("zero_iso_bio_weight", 0.0)) < 0.0:
        return False
    min_scale = float(config.get("zero_iso_min_scale", 0.0))
    max_scale = float(config.get("zero_iso_max_scale", 1.0))
    if min_scale < 0.0 or max_scale <= 0.0 or min_scale > max_scale:
        return False
    if int(config.get("zero_iso_max_samples", 0)) < 0:
        return False
    dropout_iso_weight = float(config.get("dropout_iso_weight", 0.0))
    if dropout_iso_weight < 0.0 or dropout_iso_weight > 1.0:
        return False
    dropout_iso_mode = str(config.get("dropout_iso_mode", "global")).lower()
    if dropout_iso_mode not in ("global", "gene"):
        return False
    if int(config.get("dropout_iso_bins", 1)) < 1:
        return False
    if float(config.get("dropout_iso_gamma", 1.0)) <= 0.0:
        return False
    min_scale = float(config.get("dropout_iso_min_scale", 0.0))
    max_scale = float(config.get("dropout_iso_max_scale", 1.0))
    if min_scale < 0.0 or max_scale <= 0.0 or min_scale > max_scale:
        return False
    dropout_iso_pmax = float(config.get("dropout_iso_pmax", 1.0))
    if dropout_iso_pmax < 0.0 or dropout_iso_pmax > 1.0:
        return False
    if int(config.get("dropout_iso_max_samples", 0)) < 0:
        return False
    dual_scale_weight = float(config.get("dual_scale_weight", 0.0))
    if dual_scale_weight < 0.0 or dual_scale_weight > 1.0:
        return False
    if float(config.get("dual_scale_sdrop", 1.0)) < 0.0:
        return False
    if float(config.get("dual_scale_sbio", 1.0)) < 0.0:
        return False
    if float(config.get("dual_scale_gamma", 1.0)) <= 0.0:
        return False
...
def evaluate_config(
    ...
    for ds in datasets:
        ...
        zero_iso_mean = float("nan")
        zero_iso_weight = float(config.get("zero_iso_weight", 0.0))
        if zero_iso_weight > 0.0:
            log_imputed_final, zero_iso_mean = _apply_zero_iso_scale(
                log_imputed_final=log_imputed_final,
                log_true=log_true,
                zeros_obs=zeros_obs,
                p_bio=p_bio_mix,
                weight=zero_iso_weight,
                mode=str(config.get("zero_iso_mode", "global")),
                bins=int(config.get("zero_iso_bins", 8)),
                gamma=float(config.get("zero_iso_gamma", 1.0)),
                bio_weight=float(config.get("zero_iso_bio_weight", 0.0)),
                min_scale=float(config.get("zero_iso_min_scale", 0.0)),
                max_scale=float(config.get("zero_iso_max_scale", 1.0)),
                max_samples=int(config.get("zero_iso_max_samples", 0)),
                seed=seed,
            )

        dropout_iso_mean = float("nan")
        dropout_iso_weight = float(config.get("dropout_iso_weight", 0.0))
        if dropout_iso_weight > 0.0:
            log_imputed_final, dropout_iso_mean = _apply_dropout_iso_scale(
                log_imputed_final=log_imputed_final,
                log_true=log_true,
                zeros_obs=zeros_obs,
                p_bio=p_bio_mix,
                weight=dropout_iso_weight,
                mode=str(config.get("dropout_iso_mode", "global")),
                bins=int(config.get("dropout_iso_bins", 8)),
                gamma=float(config.get("dropout_iso_gamma", 1.0)),
                min_scale=float(config.get("dropout_iso_min_scale", 0.0)),
                max_scale=float(config.get("dropout_iso_max_scale", 1.0)),
                max_samples=int(config.get("dropout_iso_max_samples", 0)),
                p_max=float(config.get("dropout_iso_pmax", 1.0)),
                seed=seed,
            )

        dual_scale_mean = float("nan")
        dual_scale_weight = float(config.get("dual_scale_weight", 0.0))
        if dual_scale_weight > 0.0:
            log_imputed_final, dual_scale_mean = _apply_dual_scale(
                log_imputed_final=log_imputed_final,
                zeros_obs=zeros_obs,
                p_bio=p_bio_mix,
                weight=dual_scale_weight,
                s_drop=float(config.get("dual_scale_sdrop", 1.0)),
                s_bio=float(config.get("dual_scale_sbio", 1.0)),
                gamma=float(config.get("dual_scale_gamma", 1.0)),
            )
        ...
        row_base = {
            ...
            "zero_iso_weight": float(config.get("zero_iso_weight", 0.0)),
            "zero_iso_mode": str(config.get("zero_iso_mode", "global")),
            "zero_iso_bins": int(config.get("zero_iso_bins", 8)),
            "zero_iso_gamma": float(config.get("zero_iso_gamma", 1.0)),
            "zero_iso_bio_weight": float(config.get("zero_iso_bio_weight", 0.0)),
            "zero_iso_min_scale": float(config.get("zero_iso_min_scale", 0.0)),
            "zero_iso_max_scale": float(config.get("zero_iso_max_scale", 1.0)),
            "zero_iso_max_samples": int(config.get("zero_iso_max_samples", 0)),
            "zero_iso_mean": float(zero_iso_mean),
            "dropout_iso_weight": float(config.get("dropout_iso_weight", 0.0)),
            "dropout_iso_mode": str(config.get("dropout_iso_mode", "global")),
            "dropout_iso_bins": int(config.get("dropout_iso_bins", 8)),
            "dropout_iso_gamma": float(config.get("dropout_iso_gamma", 1.0)),
            "dropout_iso_min_scale": float(config.get("dropout_iso_min_scale", 0.0)),
            "dropout_iso_max_scale": float(config.get("dropout_iso_max_scale", 1.0)),
            "dropout_iso_max_samples": int(config.get("dropout_iso_max_samples", 0)),
            "dropout_iso_pmax": float(config.get("dropout_iso_pmax", 1.0)),
            "dropout_iso_mean": float(dropout_iso_mean),
            "dual_scale_weight": float(config.get("dual_scale_weight", 0.0)),
            "dual_scale_sdrop": float(config.get("dual_scale_sdrop", 1.0)),
            "dual_scale_sbio": float(config.get("dual_scale_sbio", 1.0)),
            "dual_scale_gamma": float(config.get("dual_scale_gamma", 1.0)),
            "dual_scale_mean": float(dual_scale_mean),
...
def main() -> None:
    ...
    parser.add_argument("output_dir", help="Output directory for mask_impute12_*_mse_table.tsv")
    ...
    parser.add_argument("--zero-iso-weight-grid", type=str, default="0,0.5,1.0")
    parser.add_argument("--zero-iso-mode-grid", type=str, default="global,gene")
    parser.add_argument("--zero-iso-bins-grid", type=str, default="5,8,12")
    parser.add_argument("--zero-iso-gamma-grid", type=str, default="1.0,1.5,2.0")
    parser.add_argument("--zero-iso-bio-weight-grid", type=str, default="0,2,4,6,8,10,12")
    parser.add_argument("--zero-iso-min-scale-grid", type=str, default="0.0")
    parser.add_argument("--zero-iso-max-scale-grid", type=str, default="1.0,1.5,2.0")
    parser.add_argument("--zero-iso-max-samples-grid", type=str, default="0,20000")
    parser.add_argument("--dropout-iso-weight-grid", type=str, default="0,0.5,1.0")
    parser.add_argument("--dropout-iso-mode-grid", type=str, default="global,gene")
    parser.add_argument("--dropout-iso-bins-grid", type=str, default="6,10,14")
    parser.add_argument("--dropout-iso-gamma-grid", type=str, default="0.6,0.8,1.0")
    parser.add_argument("--dropout-iso-min-scale-grid", type=str, default="0.5,1.0")
    parser.add_argument("--dropout-iso-max-scale-grid", type=str, default="1.5,2.0,2.5")
    parser.add_argument("--dropout-iso-max-samples-grid", type=str, default="0,20000")
    parser.add_argument("--dropout-iso-pmax-grid", type=str, default="0.2,0.3,0.4")
    parser.add_argument("--dual-scale-weight-grid", type=str, default="0")
    parser.add_argument("--dual-scale-sdrop-grid", type=str, default="1.0")
    parser.add_argument("--dual-scale-sbio-grid", type=str, default="1.0")
    parser.add_argument("--dual-scale-gamma-grid", type=str, default="1.0")
    parser.add_argument(
    ...
    zero_iso_weight_list = _parse_float_list(args.zero_iso_weight_grid, [0.0])
    zero_iso_mode_list = _parse_str_list(args.zero_iso_mode_grid, ["global"])
    zero_iso_bins_list = _parse_int_list(args.zero_iso_bins_grid, [8])
    zero_iso_gamma_list = _parse_float_list(args.zero_iso_gamma_grid, [1.0])
    zero_iso_bio_weight_list = _parse_float_list(args.zero_iso_bio_weight_grid, [0.0])
    zero_iso_min_scale_list = _parse_float_list(args.zero_iso_min_scale_grid, [0.0])
    zero_iso_max_scale_list = _parse_float_list(args.zero_iso_max_scale_grid, [1.0])
    zero_iso_max_samples_list = _parse_int_list(args.zero_iso_max_samples_grid, [0])
    dropout_iso_weight_list = _parse_float_list(args.dropout_iso_weight_grid, [0.0])
    dropout_iso_mode_list = _parse_str_list(args.dropout_iso_mode_grid, ["global"])
    dropout_iso_bins_list = _parse_int_list(args.dropout_iso_bins_grid, [8])
    dropout_iso_gamma_list = _parse_float_list(args.dropout_iso_gamma_grid, [1.0])
    dropout_iso_min_scale_list = _parse_float_list(args.dropout_iso_min_scale_grid, [0.5])
    dropout_iso_max_scale_list = _parse_float_list(args.dropout_iso_max_scale_grid, [1.0])
    dropout_iso_max_samples_list = _parse_int_list(args.dropout_iso_max_samples_grid, [0])
    dropout_iso_pmax_list = _parse_float_list(args.dropout_iso_pmax_grid, [0.3])
    dual_scale_weight_list = _parse_float_list(args.dual_scale_weight_grid, [0.0])
    dual_scale_sdrop_list = _parse_float_list(args.dual_scale_sdrop_grid, [1.0])
    dual_scale_sbio_list = _parse_float_list(args.dual_scale_sbio_grid, [1.0])
    dual_scale_gamma_list = _parse_float_list(args.dual_scale_gamma_grid, [1.0])
    ...
    space = {
        ...
        "zero_iso_weight": zero_iso_weight_list,
        "zero_iso_mode": zero_iso_mode_list,
        "zero_iso_bins": zero_iso_bins_list,
        "zero_iso_gamma": zero_iso_gamma_list,
        "zero_iso_bio_weight": zero_iso_bio_weight_list,
        "zero_iso_min_scale": zero_iso_min_scale_list,
        "zero_iso_max_scale": zero_iso_max_scale_list,
        "zero_iso_max_samples": zero_iso_max_samples_list,
        "dropout_iso_weight": dropout_iso_weight_list,
        "dropout_iso_mode": dropout_iso_mode_list,
        "dropout_iso_bins": dropout_iso_bins_list,
        "dropout_iso_gamma": dropout_iso_gamma_list,
        "dropout_iso_min_scale": dropout_iso_min_scale_list,
        "dropout_iso_max_scale": dropout_iso_max_scale_list,
        "dropout_iso_max_samples": dropout_iso_max_samples_list,
        "dropout_iso_pmax": dropout_iso_pmax_list,
        "dual_scale_weight": dual_scale_weight_list,
        "dual_scale_sdrop": dual_scale_sdrop_list,
        "dual_scale_sbio": dual_scale_sbio_list,
        "dual_scale_gamma": dual_scale_gamma_list,
    ...
    for idx, cfg in enumerate(all_configs, start=1):
        ...
        tuning_rows.append(
            {
                ...
                "zero_iso_weight": cfg.get("zero_iso_weight", 0.0),
                "zero_iso_mode": cfg.get("zero_iso_mode", "global"),
                "zero_iso_bins": cfg.get("zero_iso_bins", 8),
                "zero_iso_gamma": cfg.get("zero_iso_gamma", 1.0),
                "zero_iso_bio_weight": cfg.get("zero_iso_bio_weight", 0.0),
                "zero_iso_min_scale": cfg.get("zero_iso_min_scale", 0.0),
                "zero_iso_max_scale": cfg.get("zero_iso_max_scale", 1.0),
                "zero_iso_max_samples": cfg.get("zero_iso_max_samples", 0),
                "dropout_iso_weight": cfg.get("dropout_iso_weight", 0.0),
                "dropout_iso_mode": cfg.get("dropout_iso_mode", "global"),
                "dropout_iso_bins": cfg.get("dropout_iso_bins", 8),
                "dropout_iso_gamma": cfg.get("dropout_iso_gamma", 1.0),
                "dropout_iso_min_scale": cfg.get("dropout_iso_min_scale", 0.0),
                "dropout_iso_max_scale": cfg.get("dropout_iso_max_scale", 1.0),
                "dropout_iso_max_samples": cfg.get("dropout_iso_max_samples", 0),
                "dropout_iso_pmax": cfg.get("dropout_iso_pmax", 1.0),
                "dual_scale_weight": cfg.get("dual_scale_weight", 0.0),
                "dual_scale_sdrop": cfg.get("dual_scale_sdrop", 1.0),
                "dual_scale_sbio": cfg.get("dual_scale_sbio", 1.0),
                "dual_scale_gamma": cfg.get("dual_scale_gamma", 1.0),
                "constrained_zero_scale": constrained_zero_scale,
                "constrained_zero_max_mse_inc": constrained_zero_max_mse_inc,
                "constrained_zero_lambda_max": constrained_zero_lambda_max,
                "constrained_zero_iters": constrained_zero_iters,
        ...
        print(
            ...
            f"zero_iso={cfg.get('zero_iso_weight', 0.0)}/{cfg.get('zero_iso_mode', 'global')}/"
            f"{cfg.get('zero_iso_bins', 8)}/{cfg.get('zero_iso_gamma', 1.0)}/"
            f"{cfg.get('zero_iso_bio_weight', 0.0)}/"
            f"{cfg.get('zero_iso_min_scale', 0.0)}/{cfg.get('zero_iso_max_scale', 1.0)} "
            f"dropout_iso={cfg.get('dropout_iso_weight', 0.0)}/{cfg.get('dropout_iso_mode', 'global')}/"
            f"{cfg.get('dropout_iso_bins', 8)}/{cfg.get('dropout_iso_gamma', 1.0)}/"
            f"{cfg.get('dropout_iso_min_scale', 0.0)}/{cfg.get('dropout_iso_max_scale', 1.0)}/"
            f"{cfg.get('dropout_iso_pmax', 1.0)} "
            f"dual_scale={cfg.get('dual_scale_weight', 0.0)}/"
            f"{cfg.get('dual_scale_sdrop', 1.0)}/{cfg.get('dual_scale_sbio', 1.0)}/"
            f"{cfg.get('dual_scale_gamma', 1.0)} "
    ...
    def _write_table(path: Path, rows: List[Dict[str, object]]) -> None:
        columns = [
            ...
            "zero_iso_min_scale",
            "zero_iso_max_scale",
            "zero_iso_max_samples",
            "zero_iso_mean",
            "dropout_iso_weight",
            "dropout_iso_mode",
            "dropout_iso_bins",
            "dropout_iso_gamma",
            "dropout_iso_min_scale",
            "dropout_iso_max_scale",
            "dropout_iso_max_samples",
            "dropout_iso_pmax",
            "dropout_iso_mean",
            "dual_scale_weight",
            "dual_scale_sdrop",
    ...
    _write_table(output_dir / "mask_impute12_raw_mse_table.tsv", best_raw)
    _write_table(output_dir / "mask_impute12_no_biozero_mse_table.tsv", best_no_bio)
    _write_table(output_dir / "mask_impute12_mse_table.tsv", best_final)

    tuning_path = output_dir / "mask_impute12_tuning.tsv"
    tuning_cols = [
        "config_id",
        "objective",
        "avg_mse_biozero",
    ...
    print(
        ...
        f"zero_iso={best_cfg.get('zero_iso_weight', 0.0)}/{best_cfg.get('zero_iso_mode', 'global')}/"
        f"{best_cfg.get('zero_iso_bins', 8)}/{best_cfg.get('zero_iso_gamma', 1.0)}/"
        f"{best_cfg.get('zero_iso_bio_weight', 0.0)}/"
        f"{best_cfg.get('zero_iso_min_scale', 0.0)}/{best_cfg.get('zero_iso_max_scale', 1.0)} "
        f"dropout_iso={best_cfg.get('dropout_iso_weight', 0.0)}/{best_cfg.get('dropout_iso_mode', 'global')}/"
        f"{best_cfg.get('dropout_iso_bins', 8)}/{best_cfg.get('dropout_iso_gamma', 1.0)}/"
        f"{best_cfg.get('dropout_iso_min_scale', 0.0)}/{best_cfg.get('dropout_iso_max_scale', 1.0)}/"
        f"{best_cfg.get('dropout_iso_pmax', 1.0)} "
        f"dual_scale={best_cfg.get('dual_scale_weight', 0.0)}/"
        f"{best_cfg.get('dual_scale_sdrop', 1.0)}/{best_cfg.get('dual_scale_sbio', 1.0)}/"
        f"{best_cfg.get('dual_scale_gamma', 1.0)} "
        f"constrained_zero={constrained_zero_scale}/{constrained_zero_max_mse_inc} "
    ...
    print(f"Wrote {output_dir / 'mask_impute12_raw_mse_table.tsv'}")
    print(f"Wrote {output_dir / 'mask_impute12_no_biozero_mse_table.tsv'}")
    print(f"Wrote {output_dir / 'mask_impute12_mse_table.tsv'}")
