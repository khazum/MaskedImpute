...
Masked AE imputation using SPLAT cell-aware (mode none, thr_drop=0.8200),
with hill-climbing over p_zero to minimize average mse_biozero across datasets.
"""
...
def train_autoencoder_reconstruct(
    ...
    for _ in range(int(epochs)):
        for xb, bio_b, nz_b in loader:
            ...
            probs = torch.zeros_like(xb)
            if p_zero > 0:
                probs = probs + bio_b * float(p_zero)
            if p_nz > 0:
                probs = probs + nz_b * float(p_nz)
...
def evaluate_p_zero(
    ...
    device: torch.device,
    seed: int,
) -> Tuple[float, List[Dict[str, object]], List[Dict[str, object]], List[Dict[str, object]]]:
    hidden = [64]
    bottleneck = 32
    ...
    for ds in datasets:
        ...
        pred_bio_mask = ds["pred_bio_mask"]
        ...
        row_base = {
            ...
            "n_obs_zero": int(np.asarray(zeros_obs, dtype=bool).sum()),
    ...
    return avg_mse_bz, results_raw, results_no_bio, results_final


def main() -> None:
    parser = argparse.ArgumentParser(description="Masked AE imputation with hill-climbing p_zero.")
    parser.add_argument("input_path", help="Path to .rds file or directory")
    parser.add_argument("output_dir", help="Output directory for mask_impute3_*_mse_table.tsv")
    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--p-nz", type=float, default=0.30, help="Mask probability for observed non-zero entries.")
    parser.add_argument("--p-zero-step", type=float, default=0.01, help="Step size for p_zero hill-climb.")
    ...
    if not (0.0 <= args.p_nz <= 1.0):
        raise ValueError("--p-nz must be within [0, 1].")
    if args.p_zero_step <= 0:
        raise ValueError("--p-zero-step must be > 0.")
    ...
    step = float(args.p_zero_step)
    p_zero = 0.0

    best_avg, best_raw, best_no_bio, best_final = evaluate_p_zero(
        datasets,
        p_zero=p_zero,
        p_nz=float(args.p_nz),
        keep_positive=bool(args.keep_positive),
        device=device,
        seed=int(args.seed),
    )
    print(f"p_zero={p_zero:.2f} avg_mse_biozero={best_avg:.6f}")

    while True:
        next_p = p_zero + step
        if next_p > 1.0 + 1e-9:
            break
        avg_mse_bz, raw, no_bio, final = evaluate_p_zero(
            datasets,
            p_zero=next_p,
            p_nz=float(args.p_nz),
            keep_positive=bool(args.keep_positive),
        ...
        print(f"p_zero={next_p:.2f} avg_mse_biozero={avg_mse_bz:.6f}")
        if avg_mse_bz < best_avg:
            p_zero = next_p
            best_avg = avg_mse_bz
    ...
    _write_table(output_dir / "mask_impute3_raw_mse_table.tsv", best_raw)
    _write_table(output_dir / "mask_impute3_no_biozero_mse_table.tsv", best_no_bio)
    _write_table(output_dir / "mask_impute3_mse_table.tsv", best_final)

    print(f"Chosen p_zero={p_zero:.2f} avg_mse_biozero={best_avg:.6f}")
    print(f"Wrote {output_dir / 'mask_impute3_raw_mse_table.tsv'}")
    print(f"Wrote {output_dir / 'mask_impute3_no_biozero_mse_table.tsv'}")
    print(f"Wrote {output_dir / 'mask_impute3_mse_table.tsv'}")
