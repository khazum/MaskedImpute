#!/usr/bin/env python3
"""
masked_imputation21.py

Supervised tuning + pruning starting from masked_imputation18.py,
using oracle biological zero probabilities from ground truth.
...
Strategy:
1) Use oracle biological zero probabilities from logTrueCounts (no tuning).
...
Parameters that improve score by <2% are dropped with their associated logic.
"""
...
def log_progress_start(
    ...
    if idx != 1 and idx != total and progress_every > 0 and idx % progress_every != 0:
        return
    print(f"[{stage}] {idx}/{total} START cfg={cfg}", flush=True)
...
def _parse_hidden_grid(text: str) -> List[List[int]]:
    if text is None:
        return []
...
def _parse_hidden_override(text: Optional[str]) -> Optional[List[int]]:
    if text is None:
        return None
...
def main() -> None:
    ...
    parser.add_argument("--ae-hidden", default=None)
    parser.add_argument("--ae-bottleneck", type=int, default=None)
    parser.add_argument("--ae-p-zero", type=float, default=None)
    parser.add_argument("--ae-p-nz", type=float, default=None)
    parser.add_argument("--ae-noise-max", type=float, default=None)
    parser.add_argument("--ae-loss-bio-weight", type=float, default=None)
    parser.add_argument("--ae-loss-nz-weight", type=float, default=None)
    parser.add_argument("--ae-bio-reg-weight", type=float, default=None)

    parser.add_argument(
        "--hidden-grid",
        default="256,256;256,128;256;128,128;128;128,64;64",
    )
    parser.add_argument("--bottleneck-grid", default="128,64,32")
    ...
    # Step 1: oracle biozero probabilities from log_true
    p_bio_list: List[np.ndarray] = []
    for ds in datasets:
        true_biozero = ds["log_true"] <= EPSILON
        p_bio = (ds["zeros_obs"] & true_biozero).astype(np.float32)
        p_bio_list.append(p_bio)
    bio_params = {"oracle_bio": True}
    ...
    hidden_list = _parse_hidden_grid(args.hidden_grid)
    if not hidden_list:
        hidden_list = [CONFIG["hidden"]]
    bottleneck_list = _parse_int_list(args.bottleneck_grid)
    if not bottleneck_list:
        bottleneck_list = [int(CONFIG["bottleneck"])]

    ae_base = {
        "epochs": 100 if args.ae_epochs is None else int(args.ae_epochs),
        "lr": 1e-3 if args.ae_lr is None else float(args.ae_lr),
        "hidden": hidden_list[0],
        "bottleneck": bottleneck_list[0],
        "p_zero": 0.0,
        "p_nz": 0.2,
        "noise_max": 0.2,
        "loss_bio_weight": 2.0,
    ...
    hidden_override = _parse_hidden_override(args.ae_hidden)
    if hidden_override is not None:
        hidden_list = [hidden_override]
        ae_base["hidden"] = hidden_override
    if args.ae_bottleneck is not None:
        bottleneck_list = [int(args.ae_bottleneck)]
        ae_base["bottleneck"] = int(args.ae_bottleneck)
    if args.ae_p_zero is not None:
        p_zero_list = [float(args.ae_p_zero)]
        ae_base["p_zero"] = float(args.ae_p_zero)
    if args.ae_p_nz is not None:
        p_nz_list = [float(args.ae_p_nz)]
        ae_base["p_nz"] = float(args.ae_p_nz)
    if args.ae_noise_max is not None:
        noise_max_list = [float(args.ae_noise_max)]
        ae_base["noise_max"] = float(args.ae_noise_max)
    if args.ae_loss_bio_weight is not None:
        loss_bio_weight_list = [float(args.ae_loss_bio_weight)]
        ae_base["loss_bio_weight"] = float(args.ae_loss_bio_weight)
    if args.ae_loss_nz_weight is not None:
        loss_nz_weight_list = [float(args.ae_loss_nz_weight)]
        ae_base["loss_nz_weight"] = float(args.ae_loss_nz_weight)
    if args.ae_bio_reg_weight is not None:
        bio_reg_weight_list = [float(args.ae_bio_reg_weight)]
        ae_base["bio_reg_weight"] = float(args.ae_bio_reg_weight)
    ...
    write_table(
        output_dir / "masked_imputation21_ae_search.tsv",
        list(ae_grid.keys()) + ["avg_mse", "avg_bz_mse", "score"],
        ae_search_rows,
    )

    write_table(
        output_dir / "masked_imputation21_ae_steps.tsv",
        [
            "param",
            "best_value",
            "avg_mse",
    ...
    write_table(
        output_dir / "masked_imputation21_post_search.tsv",
        list(post_grid.keys()) + ["avg_mse", "avg_bz_mse", "score"],
        post_search_rows,
    )

    write_table(
        output_dir / "masked_imputation21_post_steps.tsv",
        [
            "param",
            "best_value",
            "avg_mse",
    ...
    write_table(
        output_dir / "masked_imputation21_summary.tsv",
        ["stage", "avg_mse", "avg_bz_mse", "score", "details"],
        final_summary,
    )

    print("\n=== masked_imputation21 ===")
    print("Biozero mode: oracle")
    ...
    print("AE steps written to masked_imputation21_ae_steps.tsv")
    print("Postprocess steps written to masked_imputation21_post_steps.tsv")
    print("Summary written to masked_imputation21_summary.tsv")
