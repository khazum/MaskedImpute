...
`freeze_round` must accept only a direct child of `artifacts/study/`, atomically reserve its ID in an authoritative registry under Git's common directory, and reject dirty repositories, dirty/uninitialized recursive submodules, replacement refs/grafts, hidden index flags, clean-filter/config tricks, and all undeclared filesystem entries (including ignored files, empty directories, FIFOs, and sockets). Every stage-0 regular file is raw-hashed with filters disabled; symlinks and submodule gitlinks are checked explicitly. Authority roots, lock directories, registry directories, and JSON records must be owned regular/directories, non-symlinked, and opened without following links. The freeze records `method_commit`, a repository-instance ID, worktree-path hash, Git-common/state-root/registry device/inode identities, canonical round path/token, config/protocol/environment repo-relative paths and hashes, UTC time, state `frozen`, and round ID. Every transition holds the same common-directory per-round lock, verifies all authority and lock identities before each publication, and advances a lifecycle-validated digest chain with an explicit head rooted at `freeze.json`; entries are checked against the actual immutable records immediately before and after registry publication. Interrupted record-before-registry transitions are reconciled idempotently. `materialize_final` must revalidate every binding, atomically claim materialization, use `secrets.randbits(63)` to create unique generator seeds only after freeze, check the repository immediately before and after publishing them, store the seed manifest SHA-256, and transition to `materialized`. `assert_final_runnable` must revalidate HEAD, raw tracked and ignored/untracked state, all submodules, all three frozen input hashes, and the seed manifest, then atomically write the sole `execution_claim.json`; a second verifier cannot pass. `record_final_evaluation` requires that claim and revalidates the repository and seed manifest immediately before and after exclusively publishing `evaluation_receipt.json`, then transitions to `evaluated`. Files produced inside the worktree are allowed only under the canonical round's `results/` directory when each exact relative path and SHA-256 is listed in `result_manifest["result_files"]`; every other ignored entry remains a failure. Declared result files and containing directories are fsynced before receipt publication. Receipt recovery uses the receipt's manifest rather than caller input, and repository/result integrity is rechecked after the evaluated registry entry is durable. A post-publication integrity failure writes a terminal supersession record. `supersede_round` preserves every prior file and transitions to `superseded` with a nonempty reason; no in-flight caller can successfully return after supersession wins.

All JSON writes use a synchronously flushed temporary sibling followed by an exclusive hard-link publication or atomic replacement, with the containing directory fsynced after publication and temporary-file removal. Add `artifacts/` to `.gitignore`.
