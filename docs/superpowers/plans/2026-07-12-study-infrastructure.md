...
`freeze_round` must accept only a direct child of `artifacts/study/`, atomically reserve its ID in a repository-level registry, and reject dirty repositories, dirty/uninitialized recursive submodules, and index/config flags that conceal tracked changes. It then records `method_commit`, the canonical round path/token, config/protocol/environment repo-relative paths and hashes, UTC time, state `frozen`, and round ID. Every transition holds the same repository-level per-round file lock and advances the authoritative registry. `materialize_final` must revalidate every binding, atomically claim materialization, use `secrets.randbits(63)` to create unique generator seeds only after freeze, recheck the repository immediately before publishing them, store the seed manifest SHA-256, and transition to `materialized`. `assert_final_runnable` must revalidate HEAD, tracked and untracked state, all submodules, all three frozen input hashes, and the seed manifest, then atomically write the sole `execution_claim.json`; a second verifier cannot pass. `record_final_evaluation` requires that claim and revalidates the repository and seed manifest immediately before exclusively publishing `evaluation_receipt.json`, then transitions to `evaluated`. `supersede_round` preserves every prior file and transitions to `superseded` with a nonempty reason; no in-flight transition can publish after supersession returns.
