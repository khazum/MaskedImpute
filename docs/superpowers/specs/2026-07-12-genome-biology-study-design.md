...
5. The final runner requires a recursively clean Git commit (including initialized submodules), frozen config hash, protocol hash, environment hash, and unseen final manifest. Rounds are direct children of one canonical root, are reserved in a repository-level append-only state registry, and serialize transitions with per-round locks. Verification atomically claims the sole execution before a one-use evaluation receipt can be written.
