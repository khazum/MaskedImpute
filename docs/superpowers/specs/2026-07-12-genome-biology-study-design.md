...
5. The final runner requires a recursively clean Git commit (including initialized submodules), raw tracked-file bytes matching the index without clean filters or replacement objects, frozen config hash, protocol hash, environment hash, and unseen final manifest. Rounds are direct children of one canonical root, while their authoritative registry, repository-instance identity, and inode-checked locks live in Git's common directory so copied worktree artifacts cannot replay a holdout. Registry history hashes bind every immutable record. Verification atomically claims the sole execution before a one-use evaluation receipt can be written; repository integrity is checked before and after publication, and a detected race terminally supersedes the round.
