#!/usr/bin/env Rscript
# run_clustering2.R
# Usage: Rscript run_clustering2.R <input_rds_file_or_dir> <output_dir> [ncores] [n_repeats] [methods]
#
...
if (length(args) < 2) {
  stopf("Usage: Rscript run_clustering2.R <input_rds_file_or_dir> <output_dir> [ncores] [n_repeats] [methods]")
}
...
init_python_clustering <- function() {
  ...
  if (!reticulate::py_module_available("sklearn")) {
    stopf("Python env missing scikit-learn. Set MASKEDIMPUTE_PYTHON to a Python with sklearn.")
  }
  py_clust <<- tryCatch(
    reticulate::import("run_clustering2", delay_load = FALSE),
    error = function(e) stopf("Failed to import run_clustering2 via reticulate: %s", conditionMessage(e))
  )
  invisible(py_clust)
}
...
evaluate_clustering <- function(log_imp, labels, seed = 42L) {
...
for (path in files) {
  ...
  if ("baseline" %in% methods) {
    ...
    for (i in seq_len(n_repeats)) {
      ...
      res <- tryCatch({
        seed_val <- 42L + (i - 1L)
        df <- evaluate_clustering(t(logcounts), labels, seed = seed_val)
        df
      }, error = function(e) {
      ...
      if (ncol(counts_saver) < 2 || nrow(counts_saver) < 1) {
        ...
        for (i in seq_len(n_repeats)) {
          ...
          res <- tryCatch({
            ...
            df <- evaluate_clustering(t(log_imp), labels, seed = seed_val)
            df
          }, error = function(e) {
  ...
  if ("ccimpute" %in% methods) {
    ...
    for (i in seq_len(n_repeats)) {
      ...
      res <- tryCatch({
        ...
        df <- evaluate_clustering(t(as.matrix(log_imp)), labels, seed = seed_val)
        df
      }, error = function(e) {
  ...
  if ("experiment" %in% methods) {
    ...
    for (i in seq_len(n_repeats)) {
      ...
      res <- tryCatch({
        ...
        df <- evaluate_clustering(log_imp, labels, seed = seed_val)
        df
      }, error = function(e) {
