...
$\mathcal{L}=\mathcal{L}_{\mathrm{mask}}+\mathcal{L}_{\mathrm{bio}}$.
...
\begin{figure*}[t]
    ...
    learned by minimizing a weighted masked reconstruction loss plus a biozero
    regularizer.}
...
was repeated multiple times with different random seeds (10 repeats for
baselines; 5 repeats for MaskClass due to GPU budget); we report mean and
standard deviation.
...
\STATE Minimize $\mathcal{L}=\mathcal{L}_{\mathrm{mask}}+\mathcal{L}_{\mathrm{bio}}$.
\ENDFOR
\ENDFOR
...
\subsection{Denoising Accuracy on Synthetic Data ($N=5{,}000$)}
...
$\mathbf{X}^{\star}_{\log}$ (Section~\ref{sec:evaluation}). Unless stated
otherwise, \textbf{MaskClass} denotes our full model with the biozero
regularizer. Table~\ref{tab:mse_5000}
summarizes mean squared error (MSE), with parenthesized values reporting
dropout-MSE over entries that are observed as zero but non-zero in the ground
truth. MaskClass achieves the lowest MSE across all scenarios, outperforming
the strongest baseline (DCA) by 43--60\% in overall MSE and 20--49\% in
dropout-MSE. Figure~\ref{fig:mse_5000_bars} visualizes all three error
components (overall, dropout, and biozero). While MaskClass yields the best
overall and dropout-MSE, more conservative methods achieve lower biozero-MSE
(e.g., Baseline trivially attains zero biozero-MSE by leaving zeros unchanged),
highlighting a trade-off between recovering dropouts and preserving true zeros.
...
\textbf{MaskClass} & \textbf{\shortstack{0.728 $\pm$ 0.017\\ (1.417 $\pm$ 0.036)}} & \textbf{\shortstack{0.630 $\pm$ 0.012\\ (1.254 $\pm$ 0.026)}} & \textbf{\shortstack{0.688 $\pm$ 0.005\\ (1.323 $\pm$ 0.009)}} & \textbf{\shortstack{0.788 $\pm$ 0.013\\ (1.600 $\pm$ 0.029)}} \\
\bottomrule
\end{tabular}
\end{table*}
...
\subsection{Runtime Scaling on Synthetic Data}
\label{sec:runtime_scaling}
Figure~\ref{fig:runtime_scaling} reports average wall-clock runtime per dataset
across synthetic cell counts. Error bars denote the standard deviation across
synthetic datasets for each cell count.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/runtime_scaling.pdf}
\caption{Runtime scaling on synthetic datasets. We report mean wall-clock
runtime per dataset across cell counts, with error bars showing the standard
