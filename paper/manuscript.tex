...
\author*[1]{\PendingAuthor{provide the full author list and approved order}}
\affil*[1]{\PendingAuthor{provide every institutional address and author--affiliation mapping}}
\email{\PendingAuthor{provide every author email address and identify the corresponding author}}

\abstract{MaskImpute is a selective single-cell ribonucleic acid (RNA) sequencing imputation method that estimates whether each observed zero preceded technical loss and restricts reconstruction accordingly. We prespecified a truth-aware benchmark spanning four independent simulation mechanisms, paired sequencing-depth views, same-input comparators, real-data endpoints, ablations, and resource scaling. \PendingResult{insert the sealed primary result and scope-qualified conclusion}. Until then, this draft makes no empirical advance claim.}
...
All same-input methods receive the identical quality-controlled cell and gene set. Count-scale outputs are evaluated directly; normalized outputs are converted through prespecified, method-specific evaluator mappings before the common counts-per-10,000 (CP10k) $\log_2(\mathrm{CP10k}+1)$ distributional analysis. Primary reconstruction errors, dropout-restricted errors, correlation error, false-positive expression, nonzero preservation, and per-gene empirical Wasserstein distance are reported together rather than reduced to one favorable metric.

\PendingResult{insert sealed median and interquartile range (IQR) estimates, paired intervals, win/tie/loss summaries, and Pareto results}
...
The prespecified scaling panel uses 10,000, 25,000, 50,000, and 100,000 cells from one fixed mechanism. Wall-clock time, peak resident memory, peak graphics processing unit (GPU) memory, and terminal status are reported for every scheduled run. Accuracy is reported only where the truth matrix fits the prespecified metric implementation.
...
Method outputs contain only an aligned matrix and cell/gene identifiers. Endpoint targets are held in a separate evaluator object. Marker ranks and positive-control differential expression use simulator group-marker truth; Benjamini--Hochberg adjustment is applied once across the declared group-by-gene family. Clustering uses deterministic principal components and seeded $k$-means, reported as one minus the adjusted Rand index (ARI) and one minus normalized mutual information (NMI). Held-out gene and cell profile losses use independent count splits. Genuine pseudotime is required for trajectory recovery; absent truth yields a reason-coded unavailable result.
...
Artificial intelligence (AI) use requires a separate, author-approved disclosure.

\subsection{Use of generative AI or AI-assisted technologies}

\PendingAuthor{review and approve a complete disclosure of reportable large-language-model use, including its roles and the human validation and accountability applied to its outputs}

\subsection{Reproducibility and software availability}

Source revisions, source-tree hashes, runtime closure inventories, environment identities, command receipts, seeds, protocol files, and artifact checksums are machine readable. \PendingAuthor{choose and add an Open Source Initiative (OSI)-approved project license}. \PendingAuthor{provide the public repository URL and, if created, the recommended archival digital object identifier (DOI)}. The final release will include exact commands for development reconstruction, selection, freeze, one-use final execution, analysis generation, and manuscript rendering.

\section*{Abbreviations}

AI, artificial intelligence; ARI, adjusted Rand index; CP10k, counts per 10,000; DOI, digital object identifier; ERCC, External RNA Controls Consortium; GPU, graphics processing unit; IQR, interquartile range; JSON, JavaScript Object Notation; NMI, normalized mutual information; OSI, Open Source Initiative; RNA, ribonucleic acid.
...
\PendingAuthor{provide public source and data URLs, accessions, and reviewer links; if the authors create the recommended static software archive, provide its DOI or other persistent identifier}
