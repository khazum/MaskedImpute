...
MaskImpute: a biozero-aware masked autoencoder imputation method.
The method:

- uses a count-derived biozero prior only as a soft training signal,
- learns a full reconstruction model without hard-copying observed non-zeros,
- adds an auxiliary reconstruction penalty on observed expressed entries,
- retypes observed zeros from the reconstruction itself, and
- applies count-scale shrinkage only on observed zeros.
"""
...
try:
    sys.path = [
        p for p in sys.path if str(_REPO_ROOT) not in p and "MaskImpute/rds2py" not in p
    ]
...
AE_PARAMS = {
    ...
    "loss_nz_weight": 2.0,
    "bio_reg_weight": 1.2,
    # Experimental residual-denoising path. A value of 0 keeps the original
    # full-reconstruction decoder. Positive values make the decoder predict a
    # bounded correction added to an observed/corrupted anchor.
    "residual_correction_scale": 0.0,
    "residual_reg_weight": 0.0,
    # Trained residual skip alternative: the decoder still predicts a full
    # reconstruction, but the training/inference output includes a fixed
    # anchor skip. This is disabled when residual_correction_scale is positive.
    "residual_skip_weight": 0.0,
    "zero_mask_uses_bio_prob": True,
}
...
def _to_dense_array(x: object, dtype=np.float32) -> np.ndarray:
...
def _residual_denoising_enabled() -> bool:
    return float(AE_PARAMS.get("residual_correction_scale", 0.0)) > 0.0


def _residual_skip_enabled() -> bool:
    return (not _residual_denoising_enabled()) and float(AE_PARAMS.get("residual_skip_weight", 0.0)) > 0.0


def _zero_mask_source(bio_b: torch.Tensor, nz_b: torch.Tensor) -> torch.Tensor:
    if bool(AE_PARAMS.get("zero_mask_uses_bio_prob", True)):
        return bio_b
    return (1.0 - nz_b).clamp(0.0, 1.0)


def _bounded_residual(raw: torch.Tensor) -> torch.Tensor:
    return float(AE_PARAMS["residual_correction_scale"]) * torch.tanh(raw)
...
def _residual_anchor(
    ...
    mask_bio: torch.Tensor,
    mask_nz: torch.Tensor,
    zero_scaled_t: torch.Tensor,
) -> torch.Tensor:
    anchor = xb
    if mask_nz.any():
        anchor = torch.where(mask_nz.bool(), torch.zeros_like(anchor), anchor)
    if mask_bio.any():
        anchor = torch.where(mask_bio.bool(), zero_scaled_t.expand_as(anchor), anchor)
    return anchor
...
def _decode_output(
    ...
    if _residual_denoising_enabled():
        correction = _bounded_residual(raw)
        return anchor + correction, correction
    if _residual_skip_enabled():
        weight = float(AE_PARAMS["residual_skip_weight"])
        return (1.0 - weight) * raw + weight * anchor, raw - anchor
    return raw, torch.zeros((), dtype=raw.dtype, device=raw.device)
...
def _counts_from_logcounts(log_mat: np.ndarray) -> np.ndarray:
    return np.clip(np.expm1(np.asarray(log_mat, dtype=np.float32) * np.log(2.0)), 0.0, None).astype(
        np.float32
    )
...
def load_dataset(path: str) -> Dict[str, np.ndarray] | None:
    ...
    logcounts = _to_dense_array(sce.assay("logcounts").T, dtype=np.float32)
    ...
    for assay_name in ("logTrueCounts", "perfect_logcounts"):
        try:
            log_true = _to_dense_array(sce.assay(assay_name).T, dtype=np.float32)[:, keep]
            break
        except Exception:
            continue
    ...
    counts = None
    try:
        counts = _to_dense_array(sce.assay("counts").T, dtype=np.float32)
        counts = counts[:, keep]
    except Exception:
        counts = None
...
def prepare_dataset(path: Path) -> Dict[str, object] | None:
    ...
    counts_raw = dataset.get("counts")
    if counts_raw is None:
        counts_obs = np.clip(np.expm1(logcounts * np.log(2.0)), 0.0, None).astype(np.float32)
    else:
        counts_obs = np.clip(_to_dense_array(counts_raw, dtype=np.float32), 0.0, None)
    zeros_obs = counts_obs <= 0.0
    counts_max = counts_obs.max(axis=0)

    return {
        "dataset": path.parent.name if path.stem.lower() == "sce" else path.stem,
        "logcounts": logcounts,
        "log_true": log_true,
        "counts": counts_obs,
        "zeros_obs": zeros_obs,
    ...
    for _ in range(epochs):
        if use_full_gpu:
            ...
            for start in range(0, Xtr.shape[0], batch_size):
                ...
                mask_bio = torch.bernoulli(_zero_mask_source(bio_b, nz_b) * float(AE_PARAMS["p_zero"]))
                ...
                anchor = _residual_anchor(xb, mask_bio, mask_nz, zero_scaled_t)

                opt.zero_grad()
                with _autocast_ctx(bool(amp_enabled), device):
                    raw_recon = model(x_in)
                    recon, correction = _decode_output(raw_recon, anchor)
                    residual = recon - xb
                    ...
                    nz_reg = (residual.pow(2) * nz_b).sum() / nz_b.sum().clamp_min(1.0)
                    loss = (
                        masked_loss
                        + float(AE_PARAMS["bio_reg_weight"]) * bio_reg
                        + OBSERVED_RECON_WEIGHT * nz_reg
                    )
                    if _residual_denoising_enabled() and float(AE_PARAMS["residual_reg_weight"]) > 0.0:
                        loss = loss + float(AE_PARAMS["residual_reg_weight"]) * correction.pow(2).mean()
    ...
    with torch.no_grad():
        if use_full_gpu:
            for i in range(0, Xtr.size(0), batch_size):
                ...
                raw_recon = model(xb)
                recon, _ = _decode_output(raw_recon, xb)
                X01 = (recon + 1.0) / 2.0
                Z = X01 * zspan + zmin
...
def refine_bio_prob_with_reconstruction(recon_log: np.ndarray, counts_obs: np.ndarray, zeros_obs: np.ndarray) -> np.ndarray:
    """Estimate a biozero weight directly from reconstruction-scale zero evidence."""
    zero_mask = np.asarray(zeros_obs, dtype=bool)
    if not np.any(zero_mask):
        return np.zeros_like(recon_log, dtype=np.float32)

    recon_signal = _counts_from_logcounts(recon_log)
    nz_mask = counts_obs > 0.0

    recon_nz = np.where(nz_mask, recon_signal, np.nan)
    gene_ref = np.nanmedian(recon_nz, axis=0)
    fallback = np.percentile(recon_signal, 75.0, axis=0)
    gene_ref = np.where(np.isfinite(gene_ref) & (gene_ref > EPSILON), gene_ref, fallback)
    gene_ref = np.maximum(gene_ref, EPSILON).astype(np.float32)

    rel_signal = recon_signal / gene_ref[None, :]
    refined = np.asarray(1.0 / (1.0 + rel_signal), dtype=np.float32)
    refined[~zero_mask] = 0.0
    return np.clip(refined, 0.0, 1.0)


def apply_zero_gate(log_recon: np.ndarray, p_bio: np.ndarray, zeros_obs: np.ndarray) -> np.ndarray:
    """Shrink only observed zeros on count scale using the inferred biozero weight."""
    out = np.asarray(log_recon, dtype=np.float32).copy()
    zero_mask = np.asarray(zeros_obs, dtype=bool)
    if not np.any(zero_mask):
        return out

    recon_counts = np.clip(np.expm1(out * np.log(2.0)), 0.0, None).astype(np.float32)
    bio_w = np.clip(np.asarray(p_bio, dtype=np.float32), 0.0, 1.0)
    recon_counts[zero_mask] *= (1.0 - float(ZERO_SHRINK_STRENGTH) * bio_w[zero_mask])
    return np.log2(1.0 + recon_counts).astype(np.float32)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fixed-config MaskImpute biozero-aware imputation tool.")
    ...
    parser.add_argument("--bio-reg-weight", type=float, default=float(AE_PARAMS["bio_reg_weight"]))
    parser.add_argument(
        "--residual-correction-scale",
        type=float,
        default=float(AE_PARAMS["residual_correction_scale"]),
        help="Enable residual-denoising decoder with this bounded correction scale; 0 disables it.",
    )
    parser.add_argument(
        "--residual-reg-weight",
        type=float,
        default=float(AE_PARAMS["residual_reg_weight"]),
        help="L2 penalty on residual corrections when residual denoising is enabled.",
    )
    parser.add_argument(
        "--residual-skip-weight",
        type=float,
        default=float(AE_PARAMS["residual_skip_weight"]),
        help="Fixed trained anchor skip weight for full-reconstruction decoder; 0 disables it.",
    )
    ...
    AE_PARAMS["residual_correction_scale"] = float(args.residual_correction_scale)
    AE_PARAMS["residual_reg_weight"] = float(args.residual_reg_weight)
    AE_PARAMS["residual_skip_weight"] = float(args.residual_skip_weight)
    ...
    for ds in datasets:
        ...
        p_refined = refine_bio_prob_with_reconstruction(
            recon_log=recon,
            counts_obs=ds["counts"],
            zeros_obs=ds["zeros_obs"],
        )
        log_recon = apply_zero_gate(recon, p_refined, ds["zeros_obs"])
        ...
        if save_imputed:
            np.savez_compressed(
                ...
                logcounts=ds["logcounts"],
                log_reconstructed=recon,
                log_imputed=log_recon,
                p_bio=p_bio,
                p_bio_refined=p_refined,
            )
    ...
    print("\n=== MaskImpute (masked_imputation26 implementation) ===")
