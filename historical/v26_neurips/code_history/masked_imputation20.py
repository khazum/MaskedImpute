...
2) Broader search of AE parameters (no proxy labeling/post-process).
   Hidden sizes and bottleneck are selected from grids.
3) Hill-climb proxy labeling + post-process parameters one at a time.

Parameters that improve MSE by <3% are dropped with their associated logic.
"""
...
def main() -> None:
    ...
    write_table(
        output_dir / "masked_imputation20_biozero_search.tsv",
        list(bio_grid.keys()) + ["mse"],
        bio_search_rows,
    )

    write_table(
        output_dir / "masked_imputation20_biozero_steps.tsv",
    ...
    write_table(
        output_dir / "masked_imputation20_ae_search.tsv",
        list(ae_grid.keys()) + ["avg_mse", "avg_bz_mse", "score"],
        ae_search_rows,
    )

    write_table(
        output_dir / "masked_imputation20_ae_steps.tsv",
        [
            "param",
            "best_value",
            "avg_mse",
    ...
    write_table(
        output_dir / "masked_imputation20_post_search.tsv",
        list(post_grid.keys()) + ["avg_mse", "avg_bz_mse", "score"],
        post_search_rows,
    )

    write_table(
        output_dir / "masked_imputation20_post_steps.tsv",
        [
            "param",
            "best_value",
            "avg_mse",
    ...
    write_table(
        output_dir / "masked_imputation20_summary.tsv",
    ...
    print("Biozero steps written to masked_imputation20_biozero_steps.tsv")
    print("AE steps written to masked_imputation20_ae_steps.tsv")
    print("Postprocess steps written to masked_imputation20_post_steps.tsv")
    print("Summary written to masked_imputation20_summary.tsv")
