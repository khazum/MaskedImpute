...
Uses SPLAT cell-aware posterior to label observed zeros as biological vs dropout
with a fixed P(dropout) threshold, then trains a masked autoencoder where zeros
are only masked if they are predicted biological. After reconstruction, optional
post-processing keeps observed positive entries and forces predicted biological
zeros to 0 before computing MSE vs logTrueCounts.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Masked AE imputation using SPLAT cell-aware bio-zero predictions.")
    parser.add_argument("input_path", help="Path to .rds file or directory")
    parser.add_argument("output_dir", help="Output directory for mask_impute_mse_table.tsv")
    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--p-zero", type=float, default=0.01, help="Mask probability for predicted biological zeros.")
    parser.add_argument("--p-nz", type=float, default=0.30, help="Mask probability for observed non-zero entries.")
    parser.add_argument(
        "--thr-drop",
        type=float,
        default=0.7300,
        help="Predict biological if P(dropout) < thr-drop (default: 0.7300).",
    )
    ...
    if not (0.0 <= args.thr_drop <= 1.0):
        raise ValueError("--thr-drop must be within [0, 1].")
    if not (0.0 <= args.p_zero <= 1.0):
        raise ValueError("--p-zero must be within [0, 1].")
    ...
    for path in collect_rds_files(args.input_path):
        ...
        pred_bio_mask, _p_bio_z = splat_cellaware_bio_mask(
            counts_obs, zeros_obs, thr_drop=float(args.thr_drop)
        )
        ...
        row_base = {
            "dataset": ds_name,
            "thr_drop": float(args.thr_drop),
            "thr_bio": 1.0 - float(args.thr_drop),
            "p_zero": float(args.p_zero),
            "p_nz": float(args.p_nz),
    ...
    _write_table(output_dir / "mask_impute_raw_mse_table.tsv", results_raw)
    _write_table(output_dir / "mask_impute_no_biozero_mse_table.tsv", results_no_bio)
    _write_table(output_dir / "mask_impute_mse_table.tsv", results_final)

    print(f"Wrote {output_dir / 'mask_impute_raw_mse_table.tsv'}")
    print(f"Wrote {output_dir / 'mask_impute_no_biozero_mse_table.tsv'}")
    print(f"Wrote {output_dir / 'mask_impute_mse_table.tsv'}")
