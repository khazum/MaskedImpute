#!/usr/bin/env python3
"""
masked_imputation23.py

Simplified supervised tuning with fixed defaults and removed inactive logic.

Goal:
- match or improve the reference balanced performance
- remove parameters/features explicitly disabled by defaults
...
class RobustZThenMinMaxToNeg1Pos1:
    ...
    def _clip(self, X: np.ndarray) -> np.ndarray:
        return np.minimum(X, self.hi_)

    def fit(self, X: np.ndarray):
        self.hi_ = np.percentile(X, self.p_high, axis=0)
        self.lo_ = np.full_like(self.hi_, -np.inf)
        Xc = self._clip(X)
...
def main() -> None:
    ...
    parser.add_argument("--bio-disp-const-grid", default="0.05")
    parser.add_argument("--bio-use-cell-factor-grid", default="true")
    parser.add_argument("--bio-tau-dispersion-grid", default="20")
    parser.add_argument("--bio-tau-group-dispersion-grid", default="20")
    parser.add_argument("--bio-tau-dropout-grid", default="50")
    parser.add_argument("--bio-cell-zero-weight-grid", default="0.3")

    parser.add_argument("--epochs-grid", default="100")
    parser.add_argument("--lr-grid", default="0.0001")
    parser.add_argument("--p-zero-grid", default="0.0")
    parser.add_argument("--p-nz-grid", default="0.2")
    parser.add_argument("--noise-max-grid", default="0.2")
    parser.add_argument("--loss-bio-weight-grid", default="2.0")
    parser.add_argument("--loss-nz-weight-grid", default="1.0")
    parser.add_argument("--bio-reg-weight-grid", default="1.0")
    ...
    write_table(
        output_dir / "masked_imputation23_biozero_search.tsv",
        list(bio_grid.keys()) + ["mse"],
        bio_search_rows,
    )

    write_table(
        output_dir / "masked_imputation23_biozero_steps.tsv",
    ...
    write_table(
        output_dir / "masked_imputation23_ae_search.tsv",
        list(ae_grid.keys()) + ["avg_mse", "avg_bz_mse", "score"],
        ae_search_rows,
    )

    write_table(
        output_dir / "masked_imputation23_ae_steps.tsv",
        [
            "param",
            "best_value",
            "avg_mse",
    ...
    write_table(
        output_dir / "masked_imputation23_summary.tsv",
    ...
    print("Biozero steps written to masked_imputation23_biozero_steps.tsv")
    print("AE steps written to masked_imputation23_ae_steps.tsv")
    print("Summary written to masked_imputation23_summary.tsv")
