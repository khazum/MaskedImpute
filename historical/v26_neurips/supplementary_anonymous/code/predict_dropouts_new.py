...
def biozero_posterior_from_nb_dropout(p0_nb: np.ndarray, p_drop: np.ndarray) -> np.ndarray:
    """Compute P(latent count is zero | observed count is zero).

    If the latent count is zero, the observed count is zero regardless of the
    dropout indicator. Therefore the biological-zero numerator is the NB zero
    probability itself, not ``(1 - p_drop) * p0_nb``.
    """
    p0_nb = np.asarray(p0_nb, dtype=np.float64)
    p_drop = np.asarray(p_drop, dtype=np.float64)
    p_zero_obs = p_drop + (1.0 - p_drop) * p0_nb
    posterior = p0_nb / np.maximum(p_zero_obs, 1e-12)
...
def splatter_bio_posterior_from_counts(
    ...
    for group in unique_groups:
        ...
        p_bio_ij_g = biozero_posterior_from_nb_dropout(p0_nb_ij_g, p_drop_ij_g)
